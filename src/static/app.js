...
const demoOpportunities = [
  {
    event_key: "nba-finals-2025",
    total_cost: 0.975,
    edge: 0.025,
    orders: [
      { outcome: "Team A", limit_price: 0.48, size: 416.67 },
      { outcome: "Team B", limit_price: 0.495, size: 404.04 },
    ],
  },
  {
    event_key: "premier-league-title",
    total_cost: 0.987,
    edge: 0.013,
    orders: [
      { outcome: "Club X", limit_price: 0.51, size: 490.2 },
      { outcome: "Club Y", limit_price: 0.477, size: 523.06 },
    ],
  },
];
...
function renderOpportunities(opportunities) {
  grid.innerHTML = "";
  opportunities.forEach((opp) => grid.appendChild(buildCard(opp)));
}
...
async function runScan() {
  ...
  try {
    ...
    if (!data.length) {
      renderOpportunities(demoOpportunities);
      setStatus("No live opportunities found. Showing demo data.");
      return;
    }
...
renderOpportunities(demoOpportunities);
setStatus("Showing demo data. Run a scan for live markets.");
